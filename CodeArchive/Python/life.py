...
def print_grid(rows, cols, grid, generation):
    ...
    # A single output string is used to help reduce the flickering caused by
...
def create_next_grid(rows, cols, grid, next_grid):
    ...
    for row in range(rows):
        for col in range(cols):
            # Get the number of live cells adjacent to the cell at
            # grid[row][col]
            live_neighbors = get_live_neighbors(row, col, rows, cols, grid)

            # If the number of surrounding live cells is < 2 or > 3 then we
            # make the cell at grid[row][col] a dead cell
            ...
            # the cell into a live cell
                ...
                next_grid[row][col] = 1
            # If the number of surrounding live cells is 3 and the cell at
            # grid[row][col] is alive keep it alive
            else:
